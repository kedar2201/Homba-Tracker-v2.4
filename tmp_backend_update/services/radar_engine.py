...
class RadarEngine:
    ...
    def calculate_score(self, item: Tracklist, live: PriceCache, db: Session, config=None):
        ...
        if u_quality:
            ...
            # ROE (Max 10)
            if r_roe >= 25: qual_score += w_roe
            elif r_roe >= 20: qual_score += round(w_roe * 0.8)
            elif r_roe >= 15: qual_score += round(w_roe * 0.6)
            elif r_roe >= 10: qual_score += round(w_roe * 0.3)
            
            if item.sector_type in ['bank', 'nbfc']:
                # NIM (Max 5)
                w_nim = round(w_q2 * 0.5)
                if r_nim >= 4.0: qual_score += w_nim
                elif r_nim >= 3.0: qual_score += round(w_nim * 0.6)
                # Asset Quality GNPA (Max 5)
                w_gnpa = w_q2 - w_nim
                if r_gnpa < 2.0: qual_score += w_gnpa
                elif r_gnpa <= 4.0: qual_score += round(w_gnpa * 0.6)
            elif item.sector_type == 'insurance':
                # EV Growth (Max 5)
                w_ev = round(w_q2 * 0.5)
                if r_ev >= 15: qual_score += w_ev
                elif r_ev >= 10: qual_score += round(w_ev * 0.6)
                # Solvency (Max 5)
                w_solv = w_q2 - w_ev
                if r_solv >= 2.0: qual_score += w_solv
                elif r_solv >= 1.5: qual_score += round(w_solv * 0.6)
            else:
                # Standard ROCE (Max 10)
                if r_roce >= 25: qual_score += w_q2
                elif r_roce >= 20: qual_score += round(w_q2 * 0.8)
                elif r_roce >= 15: qual_score += round(w_q2 * 0.6)
                elif r_roce >= 10: qual_score += round(w_q2 * 0.3)
