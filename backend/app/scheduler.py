...
from .services.notification_service import generate_daily_portfolio_summaries, check_and_trigger_alerts
...
def start_scheduler():
    ...
    if not scheduler.running:
        ...
        # Schedule Intra-day Alert Monitor every 15 mins (Market Hours)
        trigger_alerts = CronTrigger(day_of_week='mon-fri', hour='9-15', minute='0,15,30,45')
        scheduler.add_job(
            check_and_trigger_alerts,
            trigger=trigger_alerts,
            id='intraday_alert_check',
            name='Check Alert Thresholds every 15 mins',
            replace_existing=True
        )
