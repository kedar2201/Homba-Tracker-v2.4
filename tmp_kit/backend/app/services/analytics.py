...
class AnalyticsService:
    ...
    def update_analytics(self, db: Session, symbol: str):
        ...
        try:
            ...
            # Use a custom user agent to prevent Yahoo Finance blocking the server IP
            import requests
            ...
            # A. Fetch Live Price (Using our PriceService which now uses Jugaad/NSE Live)
            try:
                # We fetch using the original symbol from our live price service
                price_data = price_service.get_prices([symbol])
                if symbol in price_data:
                    ...
                    # Fallback to fast_info if price_service failed for some reason
            ...
            # D. Fetch Technical Data (history)
            hist = ticker.history(period="1y")
