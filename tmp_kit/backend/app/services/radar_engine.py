    ...
    def calculate_score(self, item: Tracklist, live: PriceCache, db: Session, config=None):
        ...
        if u_dip and live.high_30d and live.high_30d > 0:
            ...
            if dip_pct >= item.dip_percent:
                tech_score += w_dip
                details.append(f"Dip: {dip_pct:.1f}%")
                signals_met["dip"] = True
            elif dip_pct >= (item.dip_percent * 0.75):
                tech_score += round(w_dip * 0.66)
                details.append(f"Dip: near")

        if u_rsi and live.rsi:
            if live.rsi <= item.rsi_threshold:
                tech_score += w_rsi
                details.append(f"RSI: {live.rsi:.1f}")
                signals_met["rsi"] = True
            elif live.rsi <= (item.rsi_threshold + 3):
                tech_score += round(w_rsi * 0.6)
                details.append(f"RSI: near")
        ...
        if u_dma:
            ...
            if live.ma50:
                ...
                if dist_50 <= item.near_50dma_percent:
                    tech_score += w_half
                    dma_met = True
            if live.ma200:
                dist_200 = abs(live.price - live.ma200) / live.ma200 * 100
                if dist_200 <= item.near_200dma_percent:
                    tech_score += (w_dma - w_half)
                    dma_met = True
            if dma_met:
                details.append("DMA Prox")
                signals_met["dma"] = True

        if u_breakout and item.breakout_enabled and live.high_3m and live.avg_vol_20d:
            is_breakout = live.price >= live.high_3m
            vol_spike = live.current_vol >= (live.avg_vol_20d * 1.8) if live.avg_vol_20d > 0 else False
            if is_breakout and vol_spike:
                tech_score += w_breakout
                details.append("Breakout+Vol")
                signals_met["breakout"] = True
            elif is_breakout:
                tech_score += round(w_breakout * 0.8)
                details.append("Breakout")
            elif vol_spike:
                tech_score += round(w_breakout * 0.4)
        ...
        if u_pe_disc and live.pe and live.pe_avg_5y and live.pe_avg_5y > 0:
            ...
            if disc >= 20: val_score += w_pe_disc
            elif disc >= 10: val_score += round(w_pe_disc * 0.8)
            elif disc >= 5: val_score += round(w_pe_disc * 0.4)

        if u_peg and live.peg_ratio is not None:
            if live.peg_ratio < 1.0: val_score += w_peg
            elif live.peg_ratio <= 1.5: val_score += round(w_peg * 0.6)

        # --- 3. QUALITY (20) - SECTOR AWARE ---
        if u_quality:
            # ROE (Max 10)
            if item.roe >= 25: qual_score += w_roe
            elif item.roe >= 20: qual_score += round(w_roe * 0.8)
            elif item.roe >= 15: qual_score += round(w_roe * 0.6)
            elif item.roe >= 10: qual_score += round(w_roe * 0.3)
            
            if item.sector_type in ['bank', 'nbfc']:
                # NIM (Max 5) - Shared W2 slot but we split if it's bank
                w_nim = round(w_q2 * 0.5)
                if item.nim >= 4.0: qual_score += w_nim
                elif item.nim >= 3.0: qual_score += round(w_nim * 0.6)
                # Asset Quality GNPA (Max 5)
                w_gnpa = w_q2 - w_nim
                if item.gnpa < 2.0: qual_score += w_gnpa
                elif item.gnpa <= 4.0: qual_score += round(w_gnpa * 0.6)
            elif item.sector_type == 'insurance':
                # EV Growth (Max 5)
                w_ev = round(w_q2 * 0.5)
                if item.ev_growth >= 15: qual_score += w_ev
                elif item.ev_growth >= 10: qual_score += round(w_ev * 0.6)
                # Solvency (Max 5)
                w_solv = w_q2 - w_ev
                if item.solvency_ratio >= 2.0: qual_score += w_solv
                elif item.solvency_ratio >= 1.5: qual_score += round(w_solv * 0.6)
            else:
                # Standard ROCE (Max 10)
                if item.roce >= 25: qual_score += w_q2
                elif item.roce >= 20: qual_score += round(w_q2 * 0.8)
                elif item.roce >= 15: qual_score += round(w_q2 * 0.6)
                elif item.roce >= 10: qual_score += round(w_q2 * 0.3)
