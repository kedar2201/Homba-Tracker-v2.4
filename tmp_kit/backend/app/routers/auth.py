...
def register_user(user: UserRegister, db: Session = Depends(get_db)):
    ...
    # 2. Check if device is already registered for another user to prevent trial abuse
    # (Optional: You might want to allow 1-2 users per device, but here we are strict)
    ...
    # 4. Generate OTP (Simulated for now)
    otp = str(random.randint(100000, 999999))
    new_user.otp_code = otp
    new_user.otp_expiry = datetime.utcnow() + timedelta(minutes=15)
...
def quick_price_check(symbol: str, db: Session = Depends(get_db)):
    """Fetch live price and DMA for a symbol without committing to tracklist yet."""
    from ..services.analytics import analytics_service
    from ..models.market_data import PriceCache
    ...
    # If not in cache, stale (older than 1h), or MISSING fundamentals (due to engine upgrade)
    from datetime import datetime, timedelta
    needs_refresh = (not entry 
                     or not entry.analytics_updated_at 
                     or (datetime.utcnow() - entry.analytics_updated_at > timedelta(hours=1))
                     or (entry.roe is None and entry.nim is None)) # Engine version check

    if needs_refresh:
        # Create dummy entry if missing to allow refresh
        if not entry:
    ...
    if not entry or not entry.price:
        raise HTTPException(status_code=404, detail="Price data not available for this symbol")
        
    import yfinance as yf
    # Proactive sync if fundamentals are missing or zero
    if not entry.roe or entry.roe == 0 or not entry.roce or entry.roce == 0:
        try:
            ticker = yf.Ticker(s if s.endswith(".NS") else f"{s}.NS")
            info = ticker.info
            # Direct ROE
            roe_val = info.get('returnOnEquity', 0)
            if roe_val: entry.roe = roe_val * 100
            
            # ROCE Proxy: Asset Turnover * Margin OR Return on Assets * Factor
            roa_val = info.get('returnOnAssets', 0)
            if roa_val: entry.roce = roa_val * 135 # Proxy for ROA to ROCE conversion
    ...
    logger.info(f"Price Check for {s}: ROE={entry.roe}, ROCE={entry.roce}")
