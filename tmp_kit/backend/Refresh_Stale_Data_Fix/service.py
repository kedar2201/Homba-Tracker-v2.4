    ...
    def get_prices(self, symbols: List[str], force_refresh: bool = False) -> Dict[str, Dict[str, float]]:
        ...
        try:
            ...
            for s in symbols:
                ...
                if not force_refresh and entry and entry.updated_at > expiry_time:
                    results[s] = {
                        'price': entry.price,
...
# Primary: Jugaad (NSE Community - Zero Lag)
# Fallback: YFinance (Global - 15min delay)
...
def switch_provider(provider_name: str):
    ...
    if provider_name.lower() == "broker":
        ...
        # Default for local: Jugaad primary
        price_service = PriceService(JugaadNSEProvider(), YFinanceProvider())
